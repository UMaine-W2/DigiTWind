...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    SUBROUTINE StCControl(avrSWAP, CntrPar, LocalVar, objInst)
        USE ROSCO_Types, ONLY : ControlParameters, LocalVariables, ObjectInstances
        ...
        IF (CntrPar%StC_Mode > 0) THEN
          ...
          ENDIF
          ! Send to AVRSwap
          ! Change 2801 & 2802 to their representative values in ServoDyn (according to the extended version of Bladed)
          avrSWAP(2809) = LocalVar%StC_Z_K   ! Send StC_Z_K (N/m)
          avrSWAP(2812) = LocalVar%StC_Z_C   ! Send StC_Z_C (N/(m/s))
        ELSE
          RETURN
        ENDIF
